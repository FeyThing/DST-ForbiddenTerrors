...
local builder = ParticleTileTextureBuilder(TEXTURE_INDEX_WIDTH, TEXTURE_INDEX_HEIGHT)
    ...
    if DEBUG_MODE then
        ...
        for id, tile in pairs(_id_to_tile) do
            ...
            if data ~= nil then
                local debug = _debug_grid:GetDataAtIndex(index)
                if debug == nil then
                    debug = SpawnAt("df_debug_label", Vector3(data.x, 0, data.y))
                    local tile = _tile_grid:GetDataAtIndex(index)
                    debug:SetDFDebugLabel("TILE", string.format("CACHED TILE[%s]", tile and INVERTED_WORLD_TILES[tile] or "N/A"))
                end
                local debug_str = string.format("TILE[%s](%i)", INVERTED_WORLD_TILES[_id_to_tile[id]], data.state)
                debug:SetDFDebugLabel(tostring(id), debug_str)
...
function InitializeParticleWorldTileState()
    ...
    for id, tile in pairs(_id_to_tile) do
        _tile_to_id[tile] = id
        _particletile_grids[id] = DataGrid(w, h)
    end

    if DEBUG_MODE then
        _debug_grid = DataGrid(w, h)
    end
...
function RegisterParticleWorldTileState(tile, texture, shader, config)
...
function OnAnyTerraformParticleWorldTileState(tiles_data)
    if tiles_data == nil then return end
    --dumptable(tiles_data)
    ...
    for index, data in pairs(tiles_data) do
        ...
        if GROUND_INVISIBLETILES[tile] then
            tile = _map:InternalGetTile(data.x, data.y)
        end
...
function OnTerraformParticleWorldTileState(tile_data)
    OnAnyTerraformParticleWorldTileState({[_tile_grid:GetIndex(tile_data.x, tile_data.y)] = tile_data})
end

function SetProxyParticleWorldTileState(tile, proxy_tile)
    WORLDTILE_PROXY[proxy_tile] = tile
end

function SetOverhangProxyParticleWorldTileState(tile, proxy_tile)
    OVERHANG_PROXY[proxy_tile] = tile
end

--lays out a grid of all the different visual states
function Debug_ViewWorldTileLifetimeGrid(tile, x, y)
    local id = _tile_to_id[tile]
    for i = -4, 4, 1 do
        for j = -4, 4, 1 do
            _manager:SpawnTile(x + i, y + j, (i+4)*(j+4), id)
        end
    end
end
function AddAssetsParticleWorldTileState(assets) 
