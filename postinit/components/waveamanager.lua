...
IAENV.AddComponentPostInit("wavemanager", function(self)
    local df_shimmer = {}
    
    if IsWorldDFEnabled() then
        df_shimmer[WORLD_TILES.OCEAN_EVIL] = {per_sec = 80, spawn_rate = 0, tryspawn = TrySpawnWavesOrShore}
        df_shimmer[WORLD_TILES.OCEAN_EVIL_SHORE] = {per_sec = 85, spawn_rate = 0, tryspawn = TrySpawnWavesOrShore}
    end

    for i,v in pairs(df_shimmer) do
        self.shimmer[i] = v
    end
end)