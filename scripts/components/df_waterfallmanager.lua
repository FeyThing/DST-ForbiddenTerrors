...
--------------------------------------------------------------------------
--[[ WaterfallManager class definition ]]
--------------------------------------------------------------------------
...
--     -- if prev_x ~= nil and prev_z ~= nil and player:GetDistanceSqToPoint(prev_x, 0, prev_z) < 16 then return end


--     -- prev_x, _, prev_z = player.Transform:GetWorldPosition()
--     -- _map:GetTileCenterPoint(tx, ty)
...
-- TODO (HALF): Xeno this check is not the best it could be better....
local function IsBelowGround(tile)
    return DF_OCEAN_TILES[tile] == nil and TileGroupManager:IsOceanTile(tile) or TileGroupManager:IsImpassableTile(tile)
end
...
--Register events
inst:ListenForEvent("df_local_onterraform", OnTerraform, _world)
