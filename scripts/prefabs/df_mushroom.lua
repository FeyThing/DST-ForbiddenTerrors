...
local function open(inst)
	if inst.components.pickable ~= nil and not inst.components.pickable.caninteractwith then
		...
		inst.growtask = inst:DoTaskInTime(math.random(), inst.opentaskfn)
	end
end
...
local function close(inst)
	if inst.components.pickable ~= nil and inst.components.pickable.caninteractwith then
		...
		inst.growtask = inst:DoTaskInTime(math.random(), inst.closetaskfn)
	end
end
...
local function CrazyTime(inst)
	local can_open = FindEntity(inst, 12, function(guy) return guy:HasTag("crazy") or (guy.components.sanity and guy.components.sanity:IsCrazy())
		or (TheWorld.state.moonphase == "new" and TheWorld.state.iscavenight) end)
	
	if can_open and inst.growtask == nil then
		open(inst)
	elseif not can_open and inst.growtask == nil then
		close(inst)
	end
